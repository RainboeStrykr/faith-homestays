...
A fullstack landing page for a luxury seaside hotel named **LUNAMARE**. Full-viewport video hero, a rooms & residences gallery with per-room detail pages, a services section on a second video, and a split shader / booking-form finale. Fullstack-wired reservation flow, Auth0 integration, and a relational database.
...
- **`users`** — managed by Auth0 (id, auth0Sub, name, email, avatar, role)
...
Auth0 is the auth provider. The `/login` page offers the "Sign in with Google" button. Sign-in populates the `users` table on first visit.
...
- The **"Reserve This Room"** button on each room detail page routes through Auth0 login first if the user is signed out, then submits with `roomId` + `roomType` pre-filled
...
- tRPC 11 + Hono + Drizzle ORM + Supabase (PostgreSQL) backend
- Auth0 (Google login)
- React Router v7
