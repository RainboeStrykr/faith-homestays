...
- Reservation requests (inquiry form + "Reserve This Room") persist to Supabase (PostgreSQL) via tRPC
- Auth0 sign-in; reservations associate with the signed-in user when available
...
4. Copy `.env.example` to `.env` and fill in `DATABASE_URL`, Auth0, and Admin credentials
...
- **`users`** — Auth0-managed (id, auth0Sub, name, email, avatar, role)
...
```
.
├── api/                # tRPC routers: auth, reservation. Hono server. Auth0 Integration
